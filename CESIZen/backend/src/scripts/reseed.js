/**
 * Réinitialise les pages et exercices de respiration avec les données correctes (UTF-8).
 * Exécuter : node src/scripts/reseed.js
 */
...
const pages = [
  {
    title: 'Accueil',
    slug: 'accueil',
    excerpt: 'Votre compagnon de santé mentale au quotidien',
    position: 0,
    content: `<h2>Bienvenue sur CESIZen</h2>
...
<p>L'application est accessible à tous, gratuitement, sans inscription obligatoire pour la consultation des informations.</p>`,
  },
  {
    title: 'Santé mentale',
    slug: 'sante-mentale',
    excerpt: 'Comprendre et préserver sa santé mentale',
    position: 1,
    content: `<h2>La santé mentale, qu'est-ce que c'est ?</h2>
...
<p>Il existe de nombreuses façons d'agir : exercice physique régulier, alimentation équilibrée, sommeil de qualité, activités sociales, et techniques de relaxation comme la cohérence cardiaque.</p>`,
  },
  {
    ...
    excerpt: 'Apprenez à reconnaître et gérer votre stress efficacement',
    position: 2,
    content: `<h2>Comprendre et gérer le stress</h2>
...
<p>D'autres techniques incluent la pleine conscience (mindfulness), la méditation, l'activité physique et la gestion du temps.</p>`,
  },
  {
    title: 'Prévention',
    slug: 'prevention',
    excerpt: 'Prévenez les troubles et renforcez votre bien-être mental',
    position: 3,
    content: `<h2>Prévention en santé mentale</h2>
...
<ul>
  ...
  },
  {
    title: 'À propos',
    slug: 'a-propos',
    excerpt: 'Découvrez notre mission, nos engagements et nos références',
    position: 4,
    content: `<h2>À propos de CESIZen</h2>
...
async function run() {
  console.log('Réinitialisation des données (UTF-8)...\n');

  // Supprime les anciennes données
  await pool.query('DELETE FROM breathing_exercises');
  await pool.query('ALTER TABLE breathing_exercises AUTO_INCREMENT = 1');
  await pool.query('DELETE FROM pages');
  await pool.query('ALTER TABLE pages AUTO_INCREMENT = 1');
  console.log('✓ Anciennes données supprimées');

  // Insère les exercices
  for (const ex of exercises) {
    await pool.query(
      `INSERT INTO breathing_exercises
         (name, label, inhale_duration, hold_duration, exhale_duration, description, is_active)
       VALUES (?, ?, ?, ?, ?, ?, 1)`,
      [ex.name, ex.label, ex.inhale_duration, ex.hold_duration, ex.exhale_duration, ex.description]
    );
    console.log(`✓ Exercice : ${ex.name}`);
  }

  // Insère les pages
  for (const p of pages) {
    await pool.query(
      `INSERT INTO pages (title, slug, content, excerpt, is_published, position)
       VALUES (?, ?, ?, ?, 1, ?)`,
      [p.title, p.slug, p.content, p.excerpt, p.position]
    );
    console.log(`✓ Page : ${p.title}`);
  }

  console.log('\nTerminé ! Accents correctement enregistrés.');
  process.exit(0);
}
